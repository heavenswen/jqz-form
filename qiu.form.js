...
;(function($){
	...
	/**----reg----**/
	var arrayReg = [];
		arrayReg.push({"cid":"empty","reg":/[^\s]+/});//空
		arrayReg.push({"cid":"bit_4","reg":/^[^\s]{4,10}$/});//4字节
		arrayReg.push({"cid":"pw","reg":/^[a-zA-Z|\d]\w{5,17}$/});
		arrayReg.push({"cid":"phone","reg":/^1[3|4|5|8][0-9]\d{8,8}$/});
		arrayReg.push({"cid":"chinese","reg":/^[\u4E00-\u9FFF]+$/g});
		arrayReg.push({"cid":"card","reg":/^62[0-9]{17,17}$/});
		arrayReg.push({"cid":"id","reg":/(^\d{15}$)|(^\d{17}([0-9]|X)$)/i});
		arrayReg.push({"cid":"email","reg":/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/});
		arrayReg.push({"cid":"num","reg":/^[1-9][0-9]*[\.]{0,1}[0-9]{2}$/});
		arrayReg.push({"cid":"zip_code","reg":/[1-9]\d{5}(?!\d)/});
		arrayReg.push({"cid":"day","reg":/^\d+[-|\/]\d{1,2}[-|\/]\d{1,2}/});
		arrayReg.push({"cid":"area","reg":/[^省份|地级市|市、县级市|县、区级市]/});
		arrayReg.push({"cid":"again_pw","inid":'.pw'});
		arrayReg.push({"cid":"img","reg":/^.*[^a][^b][^c]\.(?:png|jpg|bmp|gif|jpeg)$/});
	/*--var reg--*/
		if(arg.reg)
		{
			var ar = arg.reg;			
